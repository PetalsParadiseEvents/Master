...
$squareToken = isset($squareToken) ? $squareToken : '';
$squareLocId = isset($squareLocId) ? $squareLocId : 'LV04RNB7PJKCA';
...
if (!defined('CASHAPP_TAG'))               define('CASHAPP_TAG',              '$Mounikabiragoni');
...
function generateSquarePaymentUrl($orderId, $finalTotalVal) {
    ...
    try {
        ...
        if (($httpCode === 200 || $httpCode === 201) && !empty($resp)) {
            ...
            $logMsg = date('Y-m-d H:i:s') . " - Order {$orderId} - Square API Error (HTTP {$httpCode}): " . ($resp ?: $curlErr) . "\n";
