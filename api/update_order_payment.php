...
session_start();
/**
 * Admin Payment Method Update API (Internal Only)
...
try {
    ...
    $paymentMethod = isset($data['payment_method']) ? trim($data['payment_method']) : 'Unpaid';
    ...
    if ($pdo) {
        ...
        try {
            $updateStmt = $pdo->prepare("UPDATE `orders` SET `payment_method` = :payment WHERE `id` = :id");
            $updateStmt->execute([':payment' => $paymentMethod, ':id' => $orderId]);
        } catch (Exception $e) {
            // Ignored
        }
    }
